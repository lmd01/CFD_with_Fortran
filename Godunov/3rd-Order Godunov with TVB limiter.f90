...
integer,parameter::H=10,m=320,n=80!10,20,40,80,160,320,H=1,5,10
...
do j=0,n-1
!------------------------------------------------
!TVB limiter H=1,5,10
!-------------------------------------------------------
 do i=-8*n+4,2*m+8*n-4,2
 if(dabs(uan(i+1,j)-ua(i,j)).le.H*(2*dx)**2) then
    uanmod(i+1,j)=uan(i+1,j)-ua(i,j)
   else if(uan(i+1,j)-ua(i,j).gt.0 .and.ua(i+1,j)- ua(i,j).gt.0 &
  ...
  if(dabs(uap(i-1,j)-ua(i,j)).le.H*(2*dx)**2) then 
    uapmod(i-1,j)=uap(i-1,j)-ua(i,j)
   else if(ua(i,j)-uap(i-1,j).gt.0 .and.ua(i+1,j)- ua(i,j).gt.0 &
  &.and. ua(i,j)-ua(i-1,j).gt.0) then
...
!-----------------------------------------------
!output
open(111,file='10TVB3rdGodunov_m320.txt')
do i=0,2*m ,2
  write(111,*) ua(i,n)
enddo
end
